...
export async function getServerSideProps({ req }) {
  ...
  // Active registrations from Player.registeredFor
  ...
  // Load TournamentState docs for these tournaments (dynamic meta)
  let stateById = {};
  if (ids.length > 0) {
    ...
    stateById = Object.fromEntries(
      states.map((s) => [s.tournamentId, s])
    );
  }
  ...
  const registrations = rawRegs.map((r) => {
    ...
    return {
      ...
      // game label (e.g. "VALORANT", "TFT")
      game:
        state.displayGameLabel ||
        meta.game ||
        r.game ||
        "Valorant",

      // mode / format (e.g. "1v1 • Double Elimination")
      mode:
      ...
      // status: prefer TournamentState status if present
      status:
      ...
      // start time: use displayTime EXACTLY as typed by you
      // fall back to old catalog/r.start if needed
      startText:
        state.displayTime ||
        meta.start ||
        r.start ||
