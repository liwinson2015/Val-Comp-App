...
function toMatchObjects(pairs) {
  return pairs.map((pair) => ({ p1: pair?.[0] || "TBD", p2: pair?.[1] || "TBD", winner: 0 }));
}

function calcWinners(currentRound, nextRound) {
  if (!currentRound || !nextRound) return;
  currentRound.forEach((match) => {
    if (match.p1 !== "TBD" && playerInRound(match.p1, nextRound)) { match.winner = 1; } 
    else if (match.p2 && match.p2 !== "TBD" && playerInRound(match.p2, nextRound)) { match.winner = 2; }
  });
}
  ...
  return (
    <div className={s.matchWrapper}>
      <div className={`${s.matchCard} ${themeClass}`}>
        <div className={`${s.team} ${match.winner === 1 ? s.winnerRow : ""}`}><span>{match.p1}</span></div>
        {match.p2 !== undefined && (<div className={`${s.team} ${match.winner === 2 ? s.winnerRow : ""}`}><span>{match.p2}</span></div>)}
      </div>
    </div>
  );
}
  ...
  return (
    <div className={s.lbViewport}>
      <div className={s.bracketWrapper}>
        <div className={s.column}><h3 className={`${s.roundTitle} ${s.iceTitle}`}>LB Round 1</h3><div className={s.matchContainer}>{matches.R1.map((m, i) => <MatchCard key={i} match={m} theme="ice" />)}</div></div>
        <div className={s.column}><h3 className={`${s.roundTitle} ${s.iceTitle}`}>LB Round 2</h3><div className={`${s.matchContainer} ${s.connectorLeft}`}>{matches.R2.map((m, i) => <MatchCard key={i} match={m} theme="ice" />)}</div></div>
        <div className={s.column}><h3 className={`${s.roundTitle} ${s.fireTitle}`}>LB Round 3A</h3><div className={`${s.matchContainer} ${s.connectorLeft}`}>{matches.R3A.map((m, i) => <MatchCard key={i} match={m} theme="fire" />)}</div></div>
        <div className={s.column}><h3 className={`${s.roundTitle} ${s.fireTitle}`}>LB Round 3B</h3><div className={`${s.matchContainer} ${s.connectorLeft}`}>{matches.R3B.map((m, i) => <MatchCard key={i} match={m} theme="fire" />)}</div></div>
        <div className={s.column}><h3 className={`${s.roundTitle} ${s.fireTitle}`}>LB Round 4</h3><div className={`${s.matchContainer} ${s.connectorLeft}`}>{matches.R4.map((m, i) => <MatchCard key={i} match={m} theme="fire" />)}</div></div>
        
        {/* COL 6: LB FINAL (TARGET) */}
           ...
           <div className={`${s.matchContainer} ${s.connectorLeft}`}>
             <div className={s.matchWrapper}>
                ...
                <div className={`${s.matchCard} ${s.cardClash}`}>
                   <div className={`${s.team} ${s.winnerRow}`}><span>{matches.LBChamp[0].p1}</span></div>
                </div>
             </div>
           </div>
        </div>
...
function normalize(props) {
  ...
  if (d) {
    return {
      R1: ensurePairs(d.R1, 4, "TBD"), R2: mergePairs(d.R2A, d.R2B, 2, "WB R2 Loser"),
      R3A: ensurePairs(d.R3A ?? [["TBD", "TBD"], ["TBD", "TBD"]], 2, "TBD"),
      R3B: ensurePairs(d.R3B ?? [["TBD", "WB SF Loser 1"], ["TBD", "WB SF Loser 2"]], 2, "TBD"),
      R4: ensurePairs(d.R4, 1, "TBD"), LBF: Array.isArray(d.LBF) ? d.LBF : ["TBD", "WB Final Loser"],
  ...
  return { R1: ensurePairs(r1, 4, "TBD"), R2: ensurePairs(r2, 4, "TBD"), R3A: ensurePairs(r3a, 2, "TBD"), R3B: ensurePairs(r3b, 2, "TBD"), R4: ensurePairs(r4, 1, "TBD"), LBF: lbFinal, LBWinner: lbWinner };
}
